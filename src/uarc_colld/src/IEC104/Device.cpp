...
Device::Device(string identity, int id, string ip, int port, string crypt, IEC104Subsystem *parent) :
...
void *funForDataHandleThread(void *param)
{
	...
	if (done == -1)
	{
		...
		//device->close();
		device->_control->timerRestart(device->_control->_t_close);
		if (device->_thread_stop)
		{
			device->_mutex_data_handle.unlock();
			return NULL;
...
void Device::startDataHandleThread()
{
	if (_mutex_data_handle.tryLock())
	{
		...
		pthread_create(&tid, &attr, funForDataHandleThread, this);
		//_thread_data_handle.start(funForDataHandleThread, this);
	}
}
...
{
	//sendType:0--首次采集，1--采集，2--召测，3--控制(遥控/遥调)，4--调控；  1.采集不处理
	switch (sendType)
	{
	case 0:
