...
/*****模拟数据配置接口数据结构******/
struct SrcDataConfig {
	SrcDataConfig():unitId(""),itemId(0),type(0),floatMin(0),floatMax(0),expr("")
	{}
	std::string unitId;
	int itemId;
	int type;				//
	double floatMin;		//增量最小值
	double floatMax;		//增量最大值
	std::string expr;		//计算公式
	double incbase;			//增量基值
	double h[24];			//24小时模板值
...
struct ItemInfo {   //--指标配置结构体
	...
	ItemInfo(int _itemId = -1, std::string _unitId = std::string(""),
			...
			int _warnTypeMask = -1, double _baseVal=0.0, double _coefficient=0.0,
...
struct UarcStoreItemInfo{   //归集相关的指标档案信息
	UarcStoreItemInfo(std::string _unitId = std::string(""), int _itemId = -1,
							int _itemType = 0,double _baseVal = 0,double _coefficient =0.0,
...
struct  UarcProcDataItemArchive{   //--处理的数据对应指标结构体
	UarcProcDataItemArchive(std::string _unitId = std::string(""), int _itemId = -1,
							...
							double _baseVal = 0.0,double _coefficient = 0.0,
...
struct UarcCalcParaItem{     //--计算参数对应的指标档案
	UarcCalcParaItem(std::string _unitId = std::string(""),int _itemId = -1,int _itemType = 0,
			double _baseVal = 0,double _coefficent = 0.0):unitId(_unitId),
...
typedef bool RtDbMsCallBack(CRtDbMs *rd);
...
public:
   ...
   bool getArchInfo(std::string unitId,int itemId,UarcStoreItemInfo *storeItemInfo);  //获得指定的指标档案
   ...
   /*****模拟数据配置接口******/
   bool saveSrcDataConfig(SrcDataConfig &sdc);
   bool getSDC(std::vector<SrcDataConfig> &sdcs, std::string unitId = std::string(""), int itemId = -1, int type = -1);
   bool getSrcDataTimes(std::string unitId, int itemId, std::vector<time_t> &SDTs);
   bool readSrcData(std::string unitId, int itemId, time_t t, double &value);
   bool writeSrcData(std::string unitId, int itemId, time_t t, double &value);
   bool removeSrcData(std::string unitId, int itemId, time_t formTime, time_t toTime);
   bool saveDBConfig(std::string key, std::string &value);
   bool getDBConfig(std::string key, std::string &value);
   bool saveConfig(std::string name, std::string key, std::string &value);
   bool getConfig(std::string name, std::string key, std::string &value);

   bool setCallBack(std::string name, std::string key, RtDbMsCallBack callback);
